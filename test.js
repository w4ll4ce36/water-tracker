  ...
  btn.addEventListener("click",async function(){
    try{
      if(!window.SUPABASE_URL || !window.SUPABASE_ANON_KEY){
        out.textContent="❌ Supabase URL / key が設定されていません"; return;
      }
      if(!("PushManager" in window) || !("serviceWorker" in navigator)){
        out.textContent="❌ Web Pushに対応していません"; return;
      }
      let permission=Notification.permission;
      if(permission!=="granted") permission=await Notification.requestPermission();
      if(permission!=="granted"){out.textContent="❌ 通知が許可されていません";return;}

      const reg=await navigator.serviceWorker.register("./sw.js",{scope:"./"});
      await navigator.serviceWorker.ready;

      const vapid=window.VAPID_PUBLIC_KEY;
      if(!vapid){out.textContent="❌ VAPID公開鍵が未設定です";return;}
      ...
      const res=await fetch(SUPABASE_URL.replace(/\/$/,"")+"/functions/v1/save-push-subscription",{
        method:"POST",
        headers:{
